...
export default class Topbar extends React.Component {
  ...
  render () {
    ...
    if (isJson) {
      saveAsElements.push(<li key="json">
        <button type="button" onClick={this.saveAsJson}>Save as JSON</button>
      </li>)
      saveAsElements.push(<li key="yaml">
        <button type="button" onClick={this.saveAsYaml}>Convert and save as YAML</button>
      </li>)
    } else {
      saveAsElements.push(<li key="yaml">
        <button type="button" onClick={this.saveAsYaml}>Save as YAML</button>
      </li>)
      saveAsElements.push(<li key="json">
        <button type="button" onClick={this.saveAsJson}>Convert and save as JSON</button>
      </li>)
    }
      ...
      <div>
        <div className="topbar">
          <div className="topbar-wrapper">
            <Link href="#">
              ...
              <span className="topbar-logo__title">FireSwag Editor</span>
            </Link>
            <a className="topbar-logo__link" href="https://editor.swagger.io/">Powered by Swagger Editor</a>
            <DropdownMenu {...makeMenuOptions('File')}>
              <li>
                <button type="button" onClick={this.importFromURL}>Import URL</button>
              </li>
              <li>
                <button type="button" onClick={this.showModal}>Import File</button>
              </li>
            ...
            <DropdownMenu {...makeMenuOptions('Edit')}>
              <li>
                <button type="button" onClick={this.convertToYaml}>Convert to YAML</button>
              </li>
            </DropdownMenu>
            {showServersMenu ? <DropdownMenu className="long" {...makeMenuOptions('Generate Server')}>
              ...
              {this.state.servers
                .map((serv, i) => <li key={i}>
                  <button type="button" onClick={this.downloadGeneratedFile.bind(null, 'server', serv)}>{serv}</button>
                </li>)}
            </DropdownMenu> : null}
            {showClientsMenu ? <DropdownMenu className="long" {...makeMenuOptions('Generate Client')}>
              ...
              {this.state.clients
                .map((cli, i) => <li key={i}>
                  <button type="button" onClick={this.downloadGeneratedFile.bind(null, 'client', cli)}>{cli}</button>
                </li>)}
            </DropdownMenu> : null}
          </div>
        </div>
        ...
        <Modal className="modal" ref="generatorModal">
          <div className="modal-message">
            <p>
              Code generation for OAS3 is currently work in progress. The available languages is smaller than the for
              OAS/Swagger 2.0 and is constantly being updated.
            </p>
            <p>
              If you encounter issues with the existing languages, please file a ticket at&nbsp;
              <a href="https://github.com/swagger-api/swagger-codegen-generators"
                 target={'_blank'}>swagger-codegen-generators</a>. Also, as this project highly depends on community
              contributions - please consider helping us migrate the templates for other languages. Details can be found
